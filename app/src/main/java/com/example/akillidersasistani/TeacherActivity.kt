...
class TeacherActivity : AppCompatActivity() {
    private var masterSessionId: String? = null // Ortak ID'yi burada saklayacağız
    private var currentSessionCode: String? = null // Ortak ID'yi burada tutacağız
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupStatCard(R.id.cardUnderstood, "ANLAYANLAR", "#34C759")
        setupStatCard(R.id.cardNotUnderstood, "ANLAMAYANLAR", "#FF3B30")
        binding = ActivityTeacherBinding.inflate(layoutInflater)
        setContentView(binding.root)
        binding.btnGenerateQrAttendance.setOnClickListener {
            val lessonName = binding.edtLessonName.text.toString().trim()

            if (lessonName.isEmpty()) {
                binding.edtLessonName.error = "Lütfen ders adını girin!"
                return@setOnClickListener
            }

            // Eğer hoca dersi henüz başlatmadıysa (Kod üretilmediyse)
            if (this.sessionCode == null || this.sessionCode!!.isEmpty()) {
                Toast.makeText(this, "Lütfen önce dersi başlatın!", Toast.LENGTH_SHORT).show()
                // İstersen burada otomatik generateNewSession() da çağırabilirsin
            } else {
                // Üretilmiş olan ders kodunu yoklama ID'si olarak gönderiyoruz
                startQrAttendance(this.sessionCode!!, lessonName)
            }
        }
    ...
    private fun submitProjectResponse(projectId: String, message: String) {
        ...
        val responseData = mapOf(
            "studentName" to prefs.getString("name", "İsimsiz"),
            "message" to message,
            "timestamp" to com.google.firebase.database.ServerValue.TIMESTAMP
        )

        // Firebase'de ilgili projenin altına öğrenci yanıtını kaydeder
        db.child("projects").child(projectId).child("responses")
            .child(studentId).setValue(responseData)
            .addOnSuccessListener {
                // Başarılı olduğunda yapılacak işlemler (Opsiyonel)
                Log.d("Project", "Yanıt başarıyla gönderildi.")
            }
    }
    ...
    private fun startQrAttendance(sessionId: String, lessonName: String) {
        // 1. Bilgileri Profil'den çek
        val prefs = getSharedPreferences("TeacherProfile", Context.MODE_PRIVATE)
        val name = prefs.getString("teacher_name", "Eğitmen Bilgisi Yok") ?: "Eğitmen Bilgisi Yok"
        val department = prefs.getString("teacher_branch", "Bölüm Belirtilmemiş") ?: "Bölüm Belirtilmemiş"

        // 2. Firebase Referansı (Dersin kendi ID'si ile aynı yapıyoruz)
        val sessionRef = FirebaseDatabase.getInstance().getReference("attendance_sessions").child(sessionId)
        ...
        val sessionData = mapOf(
            ...
            "teacherName" to name,
            "teacherDept" to department,
            "timestamp" to ServerValue.TIMESTAMP
        )

        sessionRef.setValue(sessionData).addOnSuccessListener {
            // Bağlantı kesilirse yoklamayı kapat
            sessionRef.child("active").onDisconnect().setValue(false)

            // Web sitesini Ders ID'si ile aç
            val webUrl = "https://akilli-ders-asistani.web.app/?id=$sessionId"
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(webUrl))
            startActivity(intent)

            Toast.makeText(this, "Yoklama Başlatıldı (Kod: $sessionId)", Toast.LENGTH_LONG).show()
    ...
    private fun showProjectListDialog() {    db.child("projects").get().addOnSuccessListener { snapshot ->
        if (snapshot.exists()) {
            // "it" hatasını çözmek için tipi açıkça belirtiyoruz
            val projectTitles = snapshot.children.map { child ->
                child.child("title").value.toString()
            }.toTypedArray()

            // KRİTİK DÜZELTME: androidx.compose DEĞİL, androidx.appcompat kullanmalısın
            val builder = androidx.appcompat.app.AlertDialog.Builder(this)
            builder.setTitle("Gönderilen Projeler")

            // "which" hatasını çözmek için parametreleri isimlendiriyoruz
            builder.setItems(projectTitles) { dialog, which ->
                val selectedTitle = projectTitles[which]
                // Burada seçilen projenin detaylarını açabilirsin
                android.widget.Toast.makeText(this, "Seçilen: $selectedTitle", android.widget.Toast.LENGTH_SHORT).show()
            }

            builder.setNegativeButton("Kapat", null)
            builder.show()
        } else {
            android.widget.Toast.makeText(this, "Henüz proje oluşturulmadı.", android.widget.Toast.LENGTH_SHORT).show()
        }
    }
    }
    private fun initViews() {
        ...
        binding.btnViewProjects.setOnClickListener {
            triggerHapticFeedback(40) // Öğretmen panelindeki titreşim ismin farklı olabilir

            // Sadece listeyi açar, herhangi bir kayıt (response) göndermez
    ...
    private fun listenForStudentMessages() {val code = sessionCode ?: return

       db.child("sessions").child(code).child("messages")
            .addChildEventListener(object : ChildEventListener {
                override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                    val time = snapshot.child("timestamp").getValue(Long::class.java) ?: 1L

                    // Sadece ders başladıktan sonra gelen yeni mesajları göster
                    if (time > sessionStartTime) {
                        ...
                        runOnUiThread {
                            // iOS tarzı pencereyi açar
                            showMessagePanel(name, message, type, photo)
                        }
                    }
                }
    ...
    // 1. Fonksiyonu String parametre alacak şekilde güncelledik
    private fun generateNewSession(newCode: String) {
        ...
        btnStart.setOnClickListener {
            ...
            if (universityName.isNotEmpty() && lessonTopic.isNotEmpty() && classRoomNo.isNotEmpty()) {
                ...
                // DİKKAT: Artık burada yeni kod üretmiyoruz!
                // Dışarıdan gelen 'newCode' parametresini kullanıyoruz.
                ...
                val sessionData = mapOf(
                    ...
                    "config" to mapOf("buttonsEnabled" to true)
                )

                // Dışarıdan gelen newCode ile kayıt yapılıyor
        ...
        db.child("sessions").child(code).child("messages").addChildEventListener(object : ChildEventListener {
            override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                ...
                if (messageTime > sessionStartTime) {
                    ...
                    // YENİ: Fotoğraf verisini de alıyoruz (Artık hata vermez)
                    val photo = snapshot.child("photo").getValue(String::class.java)

                    runOnUiThread {
                        // showMessagePanel fonksiyonuna artık 4 parametre gönderiyoruz
                        showMessagePanel(name, message, type, photo)
                    }
                }
            }
    ...
    private fun showMessagePanel(name: String, message: String, type: String, photo: String?) {
        val layoutId = when (type) {
            "understood" -> R.layout.dialog_notif_understood // SENİN PAYLAŞTIĞIN YEŞİL TASARIM
            "repeat" -> R.layout.dialog_notif_repeat
            "not_understood" -> R.layout.dialog_notif_not_understood
        ...
        // Bileşenleri Bağla (Senin XML ID'lerinle birebir uyumlu)
        ...
        if (!photo.isNullOrEmpty()) {
            try {
                imgAvatar.setImageURI(android.net.Uri.parse(photo))
            } catch (e: Exception) {
                imgAvatar.setImageResource(R.drawable.ic_student_default)
            }
        } else {
            imgAvatar.setImageResource(R.drawable.ic_student_default)
        }
